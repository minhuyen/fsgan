...
def tensor2bgr(img_tensor):
    output_img = unnormalize(img_tensor.clone(), [
...
def transfer_mask(img1, img2, mask):
    mask = mask.view(mask.shape[0], 1, mask.shape[1],
                     mask.shape[2]).repeat(1, 3, 1, 1).float()
...
def create_pyramid(img, n=1):
    ...
    pyd = [img]
    for i in range(n - 1):
        pyd.append(torch.nn.functional.avg_pool2d(
            pyd[-1], 3, stride=2, padding=1, count_include_pad=False))
...
def crop2img(img, crop, bbox):
    ...
    scaled_crop = cv2.resize(
        crop, (scaled_bbox[3], scaled_bbox[2]), interpolation=cv2.INTER_CUBIC)
    ...
    crop_bbox = np.array([left, top, scaled_bbox[2] -
                         left - right, scaled_bbox[3] - top - bottom])
    ...
    out_img[scaled_bbox[1]:scaled_bbox[1] + scaled_bbox[3], scaled_bbox[0]:scaled_bbox[0] + scaled_bbox[2]] = \
        scaled_crop[crop_bbox[1]:crop_bbox[1] + crop_bbox[3],
...
def main(source_path, target_path,
    ...
    img_transforms1 = landmark_transforms.ComposePyramids(
        pil_transforms1 + tensor_transforms1)
    img_transforms2 = landmark_transforms.ComposePyramids(
        pil_transforms2 + tensor_transforms2)
    ...
    if source_bbox is None:
        raise RuntimeError(
            "Couldn't detect a face in source image: " + source_path)
    source_tensor, source_landmarks, source_bbox = img_transforms1(
        source_rgb, source_landmarks, source_bbox)
    source_cropped_bgr = tensor2bgr(source_tensor[0] if isinstance(
        source_tensor, list) else source_tensor)
    ...
    target_frame_indices, target_landmarks, target_bboxes, target_eulers = \
        extract_landmarks_bboxes_euler_from_video(
            target_path, Gp, fa, device=device)
    if target_frame_indices.size == 0:
        raise RuntimeError(
            'No faces were detected in the target video: ' + target_path)
    ...
    if output_path is not None:
        ...
        if verbose == 0:
            ...
            out_vid_size = (
                source_cropped_bgr.shape[1] * 3, source_cropped_bgr.shape[0] * 3)
    ...
    for i in tqdm(range(total_frames)):
        ...
        for j in range(len(source_tensor)):
            ...
            input_tensor.append(torch.cat(
                (source_tensor[j], frame_landmarks[j]), dim=0).unsqueeze(0).to(device))
        reenactment_img_tensor, reenactment_seg_tensor = Gr(input_tensor)
        ...
        aligned_face_mask_tensor = reenactment_seg_tensor.argmax(
            1) == 1  # face
        ...
        inpainting_input_tensor = torch.cat(
            (aligned_img_no_background_tensor, target_mask_tensor.unsqueeze(1).float()), dim=1)
        inpainting_input_tensor_pyd = create_pyramid(
            inpainting_input_tensor, len(input_tensor))
        completion_tensor = Gi(inpainting_input_tensor_pyd)

        # Blend faces
        transfer_tensor = transfer_mask(
            completion_tensor, target_img_tensor, target_mask_tensor)
        ...
        blend_input_tensor_pyd = create_pyramid(
            blend_input_tensor, len(input_tensor))
        blend_tensor = Gb(blend_input_tensor_pyd)

        # Convert back to numpy images
        reenactment_img_bgr = tensor2bgr(reenactment_img_tensor)
        reenactment_seg_bgr = tensor2bgr(blend_seg_pred(
            reenactment_img_tensor, reenactment_seg_tensor))
        target_seg_bgr = tensor2bgr(blend_seg_pred(
            target_img_tensor, target_seg_pred_tensor))
        ...
        if verbose == 0:
            if output_crop:
                render_img = np.concatenate(
                    (source_cropped_bgr, blend_img, frame_cropped_bgr), axis=1)
            else:
                render_img = crop2img(frame, blend_img, frame_bbox[0].numpy())
            # render_img = blend_img if output_crop else crop2img(frame, blend_img, frame_bbox[0].numpy())

        else:
            render_img1 = np.concatenate(
                (reenactment_img_bgr, reenactment_seg_bgr, target_seg_bgr), axis=1)
            render_img2 = np.concatenate(
                (aligned_img_no_background_bgr, completion_bgr, transfer_bgr), axis=1)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser('face_swap_image2video')
